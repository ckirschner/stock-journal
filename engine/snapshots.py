...
def history(security: dict) -> list:
    """Every snapshot ever saved for this security, newest first, each
    saying whether it was discarded and by which entry.

    The one place the raw record is walked, and so the one place that knows
    the two kinds of entry apart. Everything below reads this. A discard is
    not a row of its own here — it is a fact about the snapshot it names, and
    folding it on where it belongs is what stops a reader having to pair them
    up and get it wrong.

    An entry this build cannot place makes the whole read refuse rather than
    being skipped. The case is a journal restored from a bundle written by a
    later version, and skipping is the dangerous direction in both directions
    at once: an unrecognised discard leaves a snapshot standing that its
    owner retired, and an unrecognised kind of entry disappears from a list
    ...
    for entry in rows:
        ...
        if kind != DISCARDED:
            raise ValueError(
                ...
                f'{security.get("ticker") or "this security"} is recorded as '
                f'"{kind}", which this version of '
                "the program does not know how to read. It was most likely "
                "written by a later one. Nothing has been changed, and "
                "nothing about this security's snapshots can be shown until "
                "this version can read it.")
        target = entry.get("discards")
        if target not in taken:
            raise ValueError(
                f'A discard in {security.get("ticker") or "this security"}\'s '
                f"snapshot record names snapshot {target}, which is not on "
                "the record. Nothing has been changed.")
    ...
    return [{**e, "discarded": discarded.get(e["seq"])}
            for e in rows if e.get("kind") == TAKEN]


def standing(security: dict) -> list:
    """The snapshots that still count, newest first."""
...
def summaries(security: dict) -> list:
    """One small row per saved snapshot, newest first — enough to list them
    and choose one, and not the thing itself.

    A frozen evaluation is fifteen kilobytes of evidence and a screen that
    shipped every one of them on every render would carry megabytes to draw a
    list of dates. So the list is rows and the record is fetched when
    somebody opens one.

    Every field here is read off what was frozen and nothing is worked out
    again. A row that recomputed the state, or looked its name up in the
    strategy standing today, would be a second opinion about a record whose
    whole purpose is that it has only one.
    """
    out = []
    for row in history(security):
